...
def validaHorarioAluno(aparelhos, idade, horario, qtdeAlunosDiaTurno, qtdeProfsDiaTurno, capacidadeMaximaLocal):
    maxAparelhos = len(aparelhos)                                                                        #Variável auxiliar usada pra verificar quantos aparelhos o aluno pode usar
    cadastroAluno = True                                                                                 #Por padrão o cadastro de aluno é válido
    ...
    for i in range(6):                                                                                   #Repete 6 vezes linha 12-16
        for j in range(3):                                                                               #Repete 3 vezes linha 13-16
...
def lerMatrizHorarios():
    ...
    for i in range(6):                                                                                        #Repete 6 vezes linha 22-27
        for j in range(3):                                                                                    #Repete 3 vezes linha 23-27
            ...
            while matrizHorarios[i][j] != 0 and matrizHorarios[i][j] != 1:                                    #Enquanto a entrada for diferente de 0 e 1 repete linha 62-63
                ...
                matrizHorarios[i][j] = int(input(f'{listaDiasSemana[i]} de {listaTurnos[j]}: '))              #Entrada de 0 ou 1 pelo usuário
    return matrizHorarios                                                                                     #Retorna a matriz preenchida